...
////////////////////条件設定　ここから//////////////////////////////////////////////////////////////////////////////
	...
	double **input3=matrix(1,1000,1,1000);
	double **input4=matrix(1,1000,1,1000);
	...
	int image_wide=sosi_max+1;
	int image_high=sosi_max+1;
	//std::vector<std::vector<double>>input3;			//vectorを2次元配置にする
	//input3.resize(image_wide);
/*	input3.resize(image_wide);
	for(int i=0;i<image_high;++i){
		input3[i].resize(image_wide);
	}

	std::vector<std::vector<double>>input4;			//vectorを2次元配置にする
	input4.resize(image_wide);
	for(int i=0;i<image_high;++i){
		input4[i].resize(image_wide);
	}
	*/
	...
	input1[65][y]=0;//ここはint型でしか指定できないため，小数点を表示するには修正が必要
	input2[65][y]=0;//ここはint型でしか指定できないため，小数点を表示するには修正が必要
	}*/
	//input1[16][16]=3;
	//input2[16][16]=3;
	...
	int csv_read=1;
	double input_csv_deta_b[512];
	if(csv_read==1){
		x=1;y=1;
		char use_name[128]="real3V";
		...
		//input_csv_deta[x][y]=input_csv_deta_b[x];
			...
			input1[x][y]=3;
			input2[x][y]=3;
			input4[x][y]=input_csv_deta_b[x];
			input3[x][y]=3.2-input4[x][y];
			//printf("%d,",x);
...
////////////////////////////フォルダ作成　ここから/////////////////////////////////////////////////////////
	...
	sprintf(date_directory,"..\\result_buta\\%2d-%02d%02d-%02d%02d%02d\\",pnow->tm_year+1900,pnow->tm_mon + 1,pnow->tm_mday,pnow->tm_hour,pnow->tm_min,pnow->tm_sec);
	 ...
	 sprintf(simulation_profile,"8方向回路ネットリスト作成プログラム\n素子数%d×%d\nRm=%s,Rg=%s,Rs=%s,Rc=%s,Cs=%s\n",sosi_max,sosi_tate,Rm_parameter,Rg_parameter,Rs_parameter,Rc_parameter,Cs_parameter);
...
///////////////////////Rsの和////////////////////////////////////////////////////////////////
	...
	for(y=1;y<=sosi_tate;++y){
		for(x=1;x<=sosi_max;++x){

			if(x<sosi_max && y<sosi_tate){
...
//////////y>1,x>1 ここから/////////////////////////////////////
			...
			//よく変更する箇所
			//fprintf(fp,"V_V%d	N%d 0	pulse(0V 3.3V %fus 0.1ns 0.1ns %fus 20us)\n", parameter_num_V, i3,input3[x][y],input4[x][y]);
			fprintf(fp,"V_V%d	N%d 0	pulse(0V %d.%dV %fus 0.1ns 0.1ns %fus 20us)\n", parameter_num_V, i3,input1[x][y],input2[x][y],input3[x][y],input4[x][y]);
			//fprintf(fp,"V_V%d	N%d 0	pulse(0V %fV 0us 0.1ns 0.1ns 3.2us 20us)\n", parameter_num_V, i3,input4[x][y]);
			++i3;
...
////////////////////////////////////////////計測用終わり/////////////////////////////////////////////
	...
	free_imatrix(gaso, 1,1000,1,1000);

	free_matrix(input3, 1,1000,1,1000);
	free_matrix(input4, 1,1000,1,1000);
